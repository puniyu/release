...
#[tokio::test]
#[cfg_attr(not(feature = "docker-tests"), ignore)]
async fn git_only_release_processes_packages_with_publish_false_in_manifest() {
    use cargo_metadata::semver::Version;
    ...
    // Configure git_only = true and publish = false in release-plz config
    let config = r#"
[workspace]
git_only = true
publish = false
"#;
    ...
    assert!(
        is_tag_created(),
        "Tag should exist after release for manifest publish = false in git_only mode"
    );
}
