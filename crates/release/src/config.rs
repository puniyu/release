...
impl From<PackageConfig> for release_core::ReleaseConfig {
    fn from(value: PackageConfig) -> Self {
        let is_publish_enabled = value.publish != Some(false);
        let is_git_only = value.git_only != Some(false);
        ...
        let mut cfg = Self::default()
            .with_publish(release_core::PublishConfig::enabled(is_publish_enabled))
            .with_git_only(is_git_only)
            .with_git_release(git_release(&value))
            .with_git_tag(
                release_core::GitTagConfig::enabled(is_git_tag_enabled)
...
fn git_release(config: &PackageConfig) -> GitReleaseConfig {
    ...
    let git_release_type: release_core::ReleaseType = config
        .git_release_type
    ...
    let mut git_release = release_core::GitReleaseConfig::enabled(is_git_release_enabled)
...
impl From<PackageConfig> for release_core::UpdateConfig {
...
impl From<PackageSpecificConfig> for release_core::PackageUpdateConfig {
...
impl From<ReleaseType> for release_core::ReleaseType {
