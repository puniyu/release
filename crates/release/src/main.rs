...
use args::OutputType;
use clap::Parser;
use release_core::ReleaseRequest;
use serde::Serialize;
use tracing::error;
...
async fn run(args: CliArgs) -> anyhow::Result<()> {
    match args.command {
        Command::Update(cmd_args) => {
            ...
            let (packages_update, _temp_repo) = release_core::update(&update_request).await?;
        ...
        Command::ReleasePr(cmd_args) => {
            ...
            let release_pr = release_core::release_pr(&request).await?;
        ...
        Command::Release(cmd_args) => {
            ...
            let output = release_core::release(&request)
                .await?
                .unwrap_or_default();
        ...
        Command::SetVersion(cmd_args) => {
            ...
            release_core::set_version::set_version(&request)?;
        }
    }
    Ok(())
}
