...
async fn release_packages(
    ...
    let packages = packages_to_release(input, project);
...
fn packages_to_release<'a>(input: &ReleaseRequest, project: &'a Project) -> Vec<&'a Package> {
    project
        .workspace_packages()
        .into_iter()
        .filter(|package| should_release_package(input, package))
        .collect()
}

fn should_release_package(input: &ReleaseRequest, package: &Package) -> bool {
    package.is_publishable() || should_release_git_only_package(input, package)
}

fn should_release_git_only_package(input: &ReleaseRequest, package: &Package) -> bool {
    let config = input.get_package_config(&package.name);
    config.is_git_only() && !input.is_publish_enabled(&package.name)
}
...
fn run_cargo_publish(
    ...
    // See https://github.com/puniyu/release/issues/1545
    args.push("--package");
